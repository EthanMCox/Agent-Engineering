...
function render(): void {
  app.innerHTML = `
    <main class="app-shell">
      <header class="topbar">
        <div>
          <p class="eyebrow">Canvas Assistant</p>
            ...
            .map(
              (message, index) => `
                <article class="message ${message.role} ${message.state === "pending" ? "pending" : ""} ${
                  index === 0 && message.role === "assistant" ? "welcome" : ""
                }">
  ...
  const resetChatButton =
    document.querySelector<HTMLButtonElement>("#reset-chat");
...
function resolveRenderedMessage(payload: ChatApiResponse): {
  markdown?: string;
  fallbackText: string;
} {
  const isMarkdownContract =
    payload.content_type === "text/markdown" && payload.format === "markdown";
  if (
    isMarkdownContract &&
